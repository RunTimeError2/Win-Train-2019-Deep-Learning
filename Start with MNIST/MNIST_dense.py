...
def dense(inputs, weight_shape, name_scope, relu=False):
    with tf.name_scope(name_scope):
        weight = tf.get_variable(name=name_scope + '_weight', shape=weight_shape, initializer=tf.contrib.keras.initializers.he_normal())
        bias = tf.Variable(tf.constant(value=0.1, shape=[weight_shape[-1]], dtype=tf.float32), name=name_scope + '_bias')
        if relu:
...
class Dense_Network(object):
    def __init__(self):
        ...
        self.drop_2 = tf.nn.dropout(self.dense_layer_2, keep_prob=keep_prob)
    ...
    def eval(self, sess, input_images, input_labels):
        ...
        return sess.run([self.eval_accuracy, self.loss], feed_dict={X_image: input_images, Y_label: input_labels, keep_prob: 1.0})

    def predict(self, sess, input_images):
        # Returning the prediction for a data-batch
        return sess.run([self.output], feed_dict={X_image: input_images, keep_prob: 1.0})
